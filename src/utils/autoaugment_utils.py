...
def policy_v3():
  ...
  policy = [
      ...
      [('Brightness', 1.0, 2)], # Added --> DOESNT WORK
...
def brightness(image, factor):
  ...
  degenerate = tf.zeros_like(image, dtype=tf.int64)
  return blend(degenerate, image, factor)
...
def sharpness(image, factor):
  ...
  result = tf.where(equal, padded_degenerate, tf.cast(orig_image, dtype=tf.uint8))
...
def _apply_func_with_prob(func, image, args, prob, bboxes):
  ...
  augmented_image, augmented_bboxes = tf.cond(
      should_apply_op,
      lambda: func(tf.cast(image, dtype=tf.uint8), bboxes, *args),
      lambda: (tf.cast(image, dtype=tf.uint8), bboxes))
...
def select_and_apply_random_policy(policies, image, bboxes):
  ...
  for (i, policy) in enumerate(policies):
    image, bboxes = tf.cond(
        ...
        lambda selected_policy=policy: selected_policy(tf.cast(image, dtype=tf.uint8), bboxes),
        lambda: (tf.cast(image, dtype=tf.uint8), bboxes))
  return (image, bboxes)
