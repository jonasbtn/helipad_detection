...
def blend(image1, image2, factor):
  ...
  image1 = tf.cast(image1, dtype=tf.float64)
  image2 = tf.cast(image2, dtype=tf.float64)

  difference = image2 - image1
  scaled = factor * difference

  # Do addition in float.
  temp = tf.cast(image1, dtype=tf.float64) + scaled

  # Interpolate
  if factor > 0.0 and factor < 1.0:
    # Interpolation means we always stay within 0 and 255.
    return tf.cast(temp, tf.int64)

  # Extrapolate:
  #
  # We need to clip and then cast.
  return tf.cast(tf.clip_by_value(temp, 0.0, 255.0), tf.int64)
...
def solarize_add(image, addition=0, threshold=128):
  ...
  added_image = tf.cast(tf.clip_by_value(added_image, 0, 255), tf.int64)
...
def contrast(image, factor):
  ...
  degenerate = tf.image.grayscale_to_rgb(tf.cast(degenerate, tf.int64))
  return blend(degenerate, image, factor)
...
def random_shift_bbox(image, bbox, pixel_scaling, replace,
  ...
  image_height = tf.cast(tf.shape(image)[0], dtype=tf.float64)
  image_width = tf.cast(tf.shape(image)[1], dtype=tf.float64)
  def clip_y(val):
  ...
  minval_y = clip_y(
      min_y - tf.cast(pixel_scaling * tf.cast(bbox_height, dtype=tf.float64) / 2.0, dtype=tf.int32))
  maxval_y = clip_y(
      min_y + tf.cast(pixel_scaling * tf.cast(bbox_height, dtype=tf.float64) / 2.0, dtype=tf.int32))
  minval_x = clip_x(
      min_x - tf.cast(pixel_scaling * tf.cast(bbox_width, dtype=tf.float64) / 2.0, dtype=tf.int32))
  maxval_x = clip_x(
      min_x + tf.cast(pixel_scaling * tf.cast(bbox_width, dtype=tf.float64) / 2.0, dtype=tf.int32))
  ...
  new_bbox = tf.stack([
      tf.cast(new_min_y, dtype=tf.float64) / tf.cast(image_height, dtype=tf.float64),
      tf.cast(new_min_x, dtype=tf.float64) / tf.cast(image_width, dtype=tf.float64),
      tf.cast(new_max_y, dtype=tf.float64) / tf.cast(image_height, dtype=tf.float64),
      tf.cast(new_max_x, dtype=tf.float64) / tf.cast(image_width, dtype=tf.float64)])
...
def _rotate_bbox(bbox, image_height, image_width, degrees):
  ...
  image_height, image_width = (
      tf.cast(image_height, dtype=tf.float64), tf.cast(image_width, dtype=tf.float64))
  ...
  min_y = -tf.cast(tf.cast(image_height, dtype=tf.float64) * (bbox[0] - 0.5), dtype=tf.int32)
  min_x = -tf.cast(tf.cast(image_width, dtype=tf.float64) * (bbox[1] - 0.5), dtype=tf.int32)
  max_y = -tf.cast(tf.cast(image_height, dtype=tf.float64) * (bbox[2] - 0.5), dtype=tf.int32)
  max_x = -tf.cast(tf.cast(image_width, dtype=tf.float64) * (bbox[3] - 0.5), dtype=tf.int32)
  ...
  min_y = -(tf.cast(tf.reduce_max(new_coords[0, :]), dtype=tf.float64) / image_height - 0.5)
  min_x = tf.cast(tf.reduce_min(new_coords[1, :]), dtype=tf.float64) / image_width + 0.5
  max_y = -(tf.cast(tf.reduce_min(new_coords[0, :]), dtype=tf.float64) / image_height - 0.5)
  max_x = tf.cast(tf.reduce_max(new_coords[1, :]), dtype=tf.float64) / image_width - 0.5
...
def _shift_bbox(bbox, image_height, image_width, pixels, shift_horizontal):
  ...
  # tf_image_height = tf.to_float(image_height)
  min_y = tf.cast(tf.cast(image_height, dtype=tf.float64) * bbox[0], dtype=tf.int32)
  min_x = tf.cast(tf.cast(image_width, dtype=tf.float64) * bbox[1], dtype=tf.int32)
  max_y = tf.cast(tf.cast(image_height, dtype=tf.float64) * bbox[2], dtype=tf.int32)
  max_x = tf.cast(tf.cast(image_width, dtype=tf.float64) * bbox[3], dtype=tf.int32)

  # min_y = tf.to_int32(tf_image_height * bbox[0])
  ...
  min_y = tf.cast(min_y, dtype=tf.float64) / tf.cast(image_height, dtype=tf.float64)
  min_x = tf.cast(min_x, dtype=tf.float64) / tf.cast(image_width, dtype=tf.float64)
  max_y = tf.cast(max_y, dtype=tf.float64) / tf.cast(image_height, dtype=tf.float64)
  max_x = tf.cast(max_x, dtype=tf.float64) / tf.cast(image_width, dtype=tf.float64)
...
def translate_bbox(image, bboxes, pixels, replace, shift_horizontal):
  ...
  # pylint:enable=g-long-lambda
  bboxes = tf.cast(tf.map_fn(wrapped_shift_bbox, tf.cast(bboxes, dtype=tf.float64)), dtype=tf.double)
  return image, bboxes
...
def _shear_bbox(bbox, image_height, image_width, level, shear_horizontal):
  ...
  min_y = tf.cast(tf.reduce_min(new_coords[0, :]), dtype=tf.float64) / image_height
  min_x = tf.cast(tf.reduce_min(new_coords[1, :]), dtype=tf.float64) / image_height
  max_y = tf.cast(tf.reduce_max(new_coords[0, :]), dtype=tf.float64) / image_height
  max_x = tf.cast(tf.reduce_max(new_coords[1, :]), dtype=tf.float64) / image_height
...
def shear_with_bboxes(image, bboxes, level, replace, shear_horizontal):
  ...
  # pylint:enable=g-long-lambda
  bboxes = tf.cast(tf.map_fn(wrapped_shear_bbox, tf.cast(bboxes, dtype=tf.float64)), dtype=tf.float64)
  return image, bboxes
...
def autocontrast(image):
  ...
  def scale_channel(image):
    ...
    lo = tf.cast(tf.reduce_min(image), dtype=tf.float64)
    hi = tf.cast(tf.reduce_max(image), dtype=tf.float64)

    # Scale the image, making the lowest value 0 and the highest value 255.
    def scale_values(im):
      scale = 255.0 / (hi - lo)
      offset = -lo * scale
      im = tf.cast(im, dtype=tf.float64) * scale + offset
      im = tf.clip_by_value(im, 0.0, 255.0)
      return tf.cast(im, tf.int64)
...
def sharpness(image, factor):
  ...
  degenerate = tf.clip_by_value(degenerate, 0.0, 255.0)
  degenerate = tf.squeeze(tf.cast(degenerate, tf.int64), [0])
  ...
  equal = tf.equal(padded_mask, 1)
  # print(equal.dtype)
  # print(padded_degenerate.dtype) # uint8
  # print(orig_image.dtype)
  # result = tf.where(equal, tf.cast(padded_degenerate, dtype=tf.int32), orig_image)
  result = tf.where(equal, padded_degenerate, tf.cast(orig_image, dtype=tf.int64))
...
def _apply_func_with_prob(func, image, args, prob, bboxes):
  ...
  augmented_image, augmented_bboxes = tf.cond(
      should_apply_op,
      lambda: func(tf.cast(image, dtype=tf.int64), bboxes, *args),
      lambda: (tf.cast(image, dtype=tf.int64), bboxes))
...
def select_and_apply_random_policy(policies, image, bboxes):
  ...
  for (i, policy) in enumerate(policies):
    image, bboxes = tf.cond(
        tf.equal(i, policy_to_select), # ADD HERE THE RETURN OF THE POLICY
        lambda selected_policy=policy: selected_policy(tf.cast(image, dtype=tf.int64), bboxes),
        lambda: (tf.cast(image, dtype=tf.int64), bboxes))
  return (image, bboxes)
